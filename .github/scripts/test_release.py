...
SPEC = importlib.util.spec_from_file_location("devrig_release", SCRIPT_PATH)
...
fixes:
  - reuse matching containers

previous: v0.1.0
"""
        data = release.parse_release_notes_yaml(text)
        self.assertIn("Throwaway Postgres", data["highlights"])
        self.assertEqual(data["features"], ["first feature", "second: with colon"])
        self.assertEqual(data["fixes"], ["reuse matching containers"])
        self.assertEqual(data["previous"], "v0.1.0")

    def test_build_body_from_notes(self) -> None:
        body = release.build_release_body_from_notes(
            {
                "highlights": "First public release.",
                "features": ["Postgres up/url/down", "JSON output"],
                "fixes": ["checksum verify on macOS"],
                "changes": ["install.sh skill install"],
            },
            "v0.1.0",
            "v0.2.0",
            "owner/devrig",
        ...
        self.assertIn("### Features\n- Postgres up/url/down\n- JSON output", body)
        self.assertIn("### Fixes\n- checksum verify on macOS", body)
        self.assertIn("### Changes\n- install.sh skill install", body)
        self.assertIn(
            "https://github.com/owner/devrig/compare/v0.1.0...v0.2.0",
            body,
        )
    ...
    def test_notes_file_preferred_over_commits(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            notes_dir.mkdir()
            (notes_dir / "v0.2.0.yaml").write_text(
                "features:\n  - from yaml file\n",
                encoding="utf-8",
            )
            body = release.release_notes("v0.2.0", "owner/devrig", root=root)
...
class ReleaseBodyTests(unittest.TestCase):
    def test_groups_conventional_commits(self) -> None:
        body = release.build_release_body(
            [
                "feat: add mysql engine",
                "feat(wait): retry SELECT 1",
                "fix: reuse matching containers",
        ...
        self.assertIn("## What's new", body)
        self.assertIn("### Features\n- add mysql engine\n- retry SELECT 1", body)
        self.assertIn("### Fixes\n- reuse matching containers", body)
        self.assertIn(
            "### Other changes\n- update AGENTS.md\n- plain subject without prefix",
            body,
        )
    ...
    def test_returns_none_without_commits(self) -> None:
        self.assertIsNone(
            release.build_release_body([], "v0.1.0", "v0.2.0", "owner/devrig")
        )

    def test_first_release_has_no_compare_link(self) -> None:
        body = release.build_release_body(
            ["feat: initial"], None, "v0.1.0", "owner/devrig"
        )
        assert body is not None
    ...
    def test_skips_empty_sections(self) -> None:
        body = release.build_release_body(
            ["fix: only a fix"], "v0.1.0", "v0.2.0", "owner/devrig"
        )
        assert body is not None
