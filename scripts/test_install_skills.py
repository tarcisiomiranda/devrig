...
class InstallTests(unittest.TestCase):
    def test_copy_skill_writes_file(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            src_dir = root / "skills" / "devrig"
            src_dir.mkdir(parents=True)
            src = src_dir / "SKILL.md"
            src.write_text("---\nname: devrig\n---\nbody\n", encoding="utf-8")
            dest_dir = root / ".claude" / "skills" / "devrig"
    ...
    def test_main_installs_detected_global(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            dest = home / ".codex" / "skills" / "devrig" / "SKILL.md"
            self.assertTrue(dest.is_file())
            self.assertIn("devrig", dest.read_text(encoding="utf-8"))
